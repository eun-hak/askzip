...
## 3차 신청 준비 (2차 거절 확정일: 2026-08-06)

- 목표: **9월 중순 3차 검토 요청** (도메인 3개월+, 방문자 0 탈출 후)
- 그 전까지 애드센스 화면의 "검토 요청" 버튼 누르지 않기
- Ads.txt "찾을 수 없음" 표시는 미승인 사이트의 정상 상태로 확인됨 (파일 200 정상 서빙, 조치 불필요)
- 체크:
  - [x] 퇴직금 계산기 (08-13)
  - [x] 네이버 소유확인 메타태그 (08-13) — 서치어드바이저 sitemap 제출은 운영자 확인 필요
  - [x] 심사관 관점 개편 (08-18): 메인 히어로·계산기 섹션·신뢰 라인, 전 글 저자 카드, about 검증방식·정정정책·AI 고지, 국취제 상황별 재구성
  - [ ] 운영자 결정 대기: 프로필 사진 여부 / 네이버 블로그(외부 실체+백링크) 개설 여부
  - [ ] 빙 웹마스터 등록 (운영자)
  - [ ] A형 롱테일 5편 (실험 재료 필요)
  - [ ] 유입 그래프에서 클릭 발생 확인
...
| 2026-07-14 | **배치 3+4 조기 배포** (운영자 결정, 분산 대신 조기 완료 선택): 지원금·부동산 10편 + 조회·기타 10편 + 이미지 3장. 이로써 40편 전체 확장본 라이브 (평균 3,099자). 이후 재신청까지는 색인·유입 관찰이 핵심, 신규 글은 유입 정체 시 처방으로 |
| 2026-07-14 | **고유도 전략 시행**: 포화도 조사(20키워드) → 롱테일 신규 3편 발행(상실일 미반영·외국인 배우자 등본·재외국민 인증, 전부 근거 중심 B형) + 최고 포화 글 1편 삭제(card-point-check). 총 42편. 남은 롱테일 후보 7개는 A형(실험·캡처 동반)으로 순차 발행 예정 |
| 2026-07-20 | Search Console 실측 반영: 색인 39개·노출 69회(7/5 이후 상승)·순위 20.4 확인. 노출 잡힌 4개 글 제목 최적화(청년특례·미납 과태료·자동차 검사·국가장학금 신청 조건). 롱테일 B형 2편 추가 발행(납세증명 반영 지연·소득금액증명 발급 캘린더) → 총 44편. **재신청 기준 합의: 7월 말 클릭 발생 또는 노출 일 30+ 안착 시 신청, 늦어도 8월 첫 주 신청** |
| 2026-08-18 | **예약 발행 시스템 가동**: 롱테일 14편 사전 집필(전 편 웹 검증). 1편 즉시 발행 + 13편 예약(8/19~8/31 일 1편, publishedAt 필터 + GitHub Actions 재빌드). 발행 재개(9월) 시 총 58편 → 9월 중순 3차 신청 |
