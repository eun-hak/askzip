        ...
        <thead>
          <tr className="bg-surface">
            {headers.map((h, i) => (
              <th
                key={i}
                className="text-left px-4 py-3 text-primary font-semibold border-b border-site-border text-[13px]"
              >
                {h}
              </th>
            ))}
          ...
          {rows.map((row, ri) => (
            <tr key={ri} className="border-t border-site-border hover:bg-surface/50 transition-colors">
              {row.map((cell, ci) => (
                <td
                  key={ci}
                  className={`px-4 py-3 leading-snug ${ci === 0 ? 'text-primary font-medium' : 'text-secondary'}`}
                >
                  {cell}
                </td>
              ))}
